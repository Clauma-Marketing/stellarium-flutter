...
exports.getViewingWindow = getViewingWindow;
 ...
 */
function getSunAltitude(latitudeDeg, longitudeDeg, date) {
    const jd = getJulianDate(date);
    ...
    const sunRa = Math.atan2(Math.cos(epsilon) * Math.sin(lambda), Math.cos(lambda)) *
        RAD_TO_DEG;
...
function getNextVisibilityStart(starRaDeg, starDecDeg, latitudeDeg, longitudeDeg, fromDate) {
    ...
    let wasVisible = isVisible(starRaDeg, starDecDeg, latitudeDeg, longitudeDeg, now);
    ...
    for (let i = 1; i <= (maxSearchHours * 60) / stepMinutes; i++) {
        ...
        const nowVisible = isVisible(starRaDeg, starDecDeg, latitudeDeg, longitudeDeg, checkTime);
        if (!wasVisible && nowVisible) {
            // Star just became visible - refine to find exact time
            return refineVisibilityStart(starRaDeg, starDecDeg, latitudeDeg, longitudeDeg, new Date(checkTime.getTime() - stepMinutes * 60 * 1000), checkTime);
        }
 ...
 */
function refineVisibilityStart(starRaDeg, starDecDeg, latitudeDeg, longitudeDeg, notVisibleTime, visibleTime) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (isVisible(starRaDeg, starDecDeg, latitudeDeg, longitudeDeg, midTime)) {
            high = mid;
        }
        else {
            low = mid;
...
function getVisibilityEnd(starRaDeg, starDecDeg, latitudeDeg, longitudeDeg, fromDate) {
    ...
    for (let i = 1; i <= (maxSearchHours * 60) / stepMinutes; i++) {
        ...
        const stillVisible = isVisible(starRaDeg, starDecDeg, latitudeDeg, longitudeDeg, checkTime);
        if (!stillVisible) {
            return checkTime;
        }
    }
 ...
 */
function getViewingWindow(starRaDeg, starDecDeg, latitudeDeg, longitudeDeg, fromDate) {
    const now = fromDate || new Date();
