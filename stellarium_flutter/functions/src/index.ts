 ...
 */
export const checkStarVisibility = onSchedule(
  {
  ...
  async () => {
    ...
    try {
      ...
      for (const userDoc of usersSnapshot.docs) {
        try {
          ...
          for (const starDoc of starsSnapshot.docs) {
            const star = starDoc.data() as SavedStar;
            ...
            const sent = await checkAndNotifyForStar(
              userDoc.id,
              starDoc.id,
              star,
              userData
      ...
      console.log(
        `Visibility check complete. Sent ${notificationsSent} notifications, ${errors} errors.`
      );
    } catch (error) {
 ...
 */
async function checkAndNotifyForStar(
  userId: string,
  starId: string,
  star: SavedStar,
  userData: UserData
  ...
  const nextVisibility = getNextVisibilityStart(
    star.ra!,
    star.dec!,
    userData.latitude!,
    userData.longitude!,
  ...
  if (currentlyVisible) {
    ...
    const minutesUntilVisible =
      (nextVisibility.getTime() - now.getTime()) / (1000 * 60);
  ...
  try {
    const direction = getDirectionName(
      getStarAzimuth(
        star.ra!,
        star.dec!,
        userData.latitude!,
...
export const triggerVisibilityCheck = onRequest(
  {
    cors: true,
    region: "europe-west1",
  },
