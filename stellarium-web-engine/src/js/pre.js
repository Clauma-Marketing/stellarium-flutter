...
/*
 * Function: setSelectionCustomLabel
 * Set a custom label to display for the currently selected object.
 * The label will be rendered in the sky view next to the selected star.
 *
 * Parameters:
 *   label  - The custom label to display, or null/empty to clear.
 */
Module['setSelectionCustomLabel'] = function(label) {
  var set_label = Module.cwrap('core_set_selection_custom_label', null, ['string']);
  set_label(label || '');
}

/*
 * Function: getSelectionCustomLabel
 * Get the custom label for the currently selected object.
 *
 * Return:
 *   The custom label string, or null if none is set.
 */
Module['getSelectionCustomLabel'] = function() {
  var get_label = Module.cwrap('core_get_selection_custom_label', 'number', []);
  var cret = get_label();
...
/*
 * Function: addPersistentLabel
 * Add a persistent custom label for a star (shown without selection).
 *
 * Parameters:
 *   identifier - Star identifier (e.g., "HIP 14778")
 *   label      - Custom label to display
 */
Module['addPersistentLabel'] = function(identifier, label) {
  var add_label = Module.cwrap('core_add_persistent_label', null, ['string', 'string']);
  add_label(identifier || '', label || '');
}

/*
 * Function: removePersistentLabel
 * Remove a persistent custom label for a star.
 *
 * Parameters:
 *   identifier - Star identifier to remove label for
 */
Module['removePersistentLabel'] = function(identifier) {
  var remove_label = Module.cwrap('core_remove_persistent_label', null, ['string']);
  remove_label(identifier || '');
}

/*
 * Function: getPersistentLabel
 * Get the persistent custom label for a star by identifier.
 *
 * Parameters:
 *   identifier - Star identifier (e.g., "HIP 14778")
 *
 * Return:
 *   The custom label string, or null if none is set for this star.
 */
Module['getPersistentLabel'] = function(identifier) {
  var get_label = Module.cwrap('core_get_persistent_label', 'number', ['string']);
  var cret = get_label(identifier || '');
  if (cret === 0) return null;
  return Module.UTF8ToString(cret);
}

/*
 * Function: clearPersistentLabels
 * Remove all persistent custom labels.
 */
Module['clearPersistentLabels'] = function() {
  var clear_labels = Module.cwrap('core_clear_persistent_labels', null, []);
  clear_labels();
}

/*
 * Function: getTargetPos
 * Get the position of the currently selected object in the OBSERVED frame.
 * This is useful for gyroscope mode to calculate direction to target.
 *
 * Return:
 *   A 3D vector [x, y, z] in the OBSERVED frame, or null if no selection.
 */
Module['getTargetPos'] = function() {
  var selection = Module.core.selection;
  if (!selection) return null;
  var radec = selection.getInfo('radec');
  if (!radec) return null;
  // Convert from ICRF to OBSERVED frame
  var observed = Module.convertFrame(Module.observer, 'ICRF', 'OBSERVED', radec);
  return observed;
}

/*
 * Function: getScreenPos
 * Get the screen position of an object.
 *
 * Parameters:
 *   obj - A SweObj object
 *
 * Return:
 *   A 2D array [x, y] in screen coordinates, or null if not visible.
 */
Module['getScreenPos'] = function(obj) {
  if (!obj) return null;
  // Use the C function to get 2D ellipse which includes screen position
  var ptr = Module._malloc(5 * 8); // win_pos[2], win_size[2], angle
  Module._obj_get_2d_ellipse(obj.v, Module.observer.v, 0, ptr, ptr + 16, ptr + 32);
  var x = Module._getValue(ptr, 'double');
  var y = Module._getValue(ptr + 8, 'double');
  Module._free(ptr);
  return [x, y];
}
