...
{
    free(core->selection_custom_label);
    core->selection_custom_label = label && *label ? strdup(label) : NULL;
}

EMSCRIPTEN_KEEPALIVE
const char *core_get_selection_custom_label(void)
{
    return core->selection_custom_label;
}

// Helper to normalize identifier for comparison (remove spaces, uppercase)
static void normalize_identifier(const char *src, char *dst, int dst_size)
{
    int i = 0;
...
{
    if (!identifier || !*identifier || !label || !*label) return;

    // Check if already exists, update if so
    persistent_label_t *pl;
    char norm_id[64], norm_existing[64];
    normalize_identifier(identifier, norm_id, sizeof(norm_id));

    for (pl = core->persistent_labels; pl; pl = pl->next) {
        normalize_identifier(pl->identifier, norm_existing, sizeof(norm_existing));
        if (strcmp(norm_id, norm_existing) == 0) {
            // Update existing label
            free(pl->label);
            pl->label = strdup(label);
            return;
        }
    }
    ...
    pl->identifier = strdup(identifier);
    pl->label = strdup(label);
    pl->next = core->persistent_labels;
    core->persistent_labels = pl;
}
...
void core_remove_persistent_label(const char *identifier)
{
    ...
    persistent_label_t *pl, *prev = NULL;
    char norm_id[64], norm_existing[64];
    normalize_identifier(identifier, norm_id, sizeof(norm_id));

    for (pl = core->persistent_labels; pl; prev = pl, pl = pl->next) {
        normalize_identifier(pl->identifier, norm_existing, sizeof(norm_existing));
        if (strcmp(norm_id, norm_existing) == 0) {
            if (prev)
                prev->next = pl->next;
            else
                core->persistent_labels = pl->next;
            free(pl->identifier);
            free(pl->label);
            free(pl);
            return;
...
{
    persistent_label_t *pl, *next;
    for (pl = core->persistent_labels; pl; pl = next) {
        next = pl->next;
        free(pl->identifier);
        free(pl->label);
        free(pl);
    }
    core->persistent_labels = NULL;
}
...
static obj_klass_t core_klass = {
    ...
    .attributes = (attribute_t[]) {
        ...
        PROPERTY(gyroscope_mode, TYPE_BOOL, MEMBER(core_t, gyroscope_mode)),
        {}
    }
};
