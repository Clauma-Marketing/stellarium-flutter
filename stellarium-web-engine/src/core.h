...
typedef struct persistent_label persistent_label_t;

extern core_t *core;    // Global core object.

/*
 * Type: persistent_label_t
 * A persistent custom label for a star (shown without selection)
 */
struct persistent_label
{
    persistent_label_t *next;
    char *identifier;  // Star identifier (e.g., "HIP 14778")
    char *label;       // Custom label to display
};
...
struct core
{
    ...
    obj_t           *selection;
    char            *selection_custom_label; // Custom label for selected object
    persistent_label_t *persistent_labels;   // Linked list of persistent labels
    ...
    // Gyroscope mode - when true, render direction arrow to selection
    bool gyroscope_mode;
};
...
/*
 * Function: core_set_selection_custom_label
 * Set a custom label to display for the currently selected object.
 *
 * Parameters:
 *   label  - The custom label to display, or NULL to clear.
 */
void core_set_selection_custom_label(const char *label);

/*
 * Function: core_get_selection_custom_label
 * Get the custom label for the currently selected object.
 *
 * Return:
 *   The custom label, or NULL if none is set.
 */
const char *core_get_selection_custom_label(void);

/*
 * Function: core_add_persistent_label
 ...
 */
void core_add_persistent_label(const char *identifier, const char *label);

/*
 * Function: core_remove_persistent_label
 * Remove a persistent custom label for a star.
 *
 * Parameters:
 *   identifier - Star identifier to remove label for
 */
void core_remove_persistent_label(const char *identifier);

/*
 * Function: core_get_persistent_label
 ...
 *
 * Return:
 *   The custom label, or NULL if none is set for this star.
 */
const char *core_get_persistent_label(const char *identifier);

/*
 * Function: core_clear_persistent_labels
 * Remove all persistent custom labels.
 */
void core_clear_persistent_labels(void);
