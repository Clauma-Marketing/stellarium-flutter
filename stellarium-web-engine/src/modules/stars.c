                             ...
                             double color[3])
{
    ...
    // Custom gold color for custom labels
    static const double gold[4] = {1.0, 0.84, 0.0, 1.0};
    ...
    const char *persistent_label = NULL;
    ...
    // Check for custom label on selected star
    if (selected) {
        custom_label = core_get_selection_custom_label();
    }

    // Check for persistent label by HIP number
    if (s->hip) {
        snprintf(hip_buf, sizeof(hip_buf), "HIP %d", s->hip);
        persistent_label = core_get_persistent_label(hip_buf);
    }

    // If we have a custom label for the selected star, use it directly
    if (custom_label && *custom_label) {
        snprintf(buf, sizeof(buf), "%s", custom_label);
    ...
    // If we have a persistent label, use it (always show, even unselected)
    if (persistent_label && *persistent_label) {
        snprintf(buf, sizeof(buf), "%s", persistent_label);
        vec4_copy(gold, label_color);
        effects = TEXT_BOLD;
        radius += LABEL_SPACING;
        u8_split_line(buf, sizeof(buf), buf, 16);
